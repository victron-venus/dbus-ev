...
class AppVersionManager:
    ...
    def websocket_user_agent(self) -> str:
        ...
        if self._profile.websocket_user_agent_template:
            return self._profile.websocket_user_agent_template.format(
    ...
    async def async_refresh(self, session: ClientSession, force: bool = False) -> bool:
        ...
        if (
            not force
            and (time.monotonic() - self._last_check_monotonic) < APP_VERSION_CHECK_INTERVAL_SECONDS
        ):
            return False
        ...
        async with self._lock:
            if (
                not force
                and (time.monotonic() - self._last_check_monotonic)
                < APP_VERSION_CHECK_INTERVAL_SECONDS
            ):
                return False
            ...
            new_version = await self._lookup_app_store_version(
                session, force_update.get("storeUrl")
            )
            if not new_version:
                LOGGER.warning(
                    (
