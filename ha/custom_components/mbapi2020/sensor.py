# Preserve upstream entity presentation; dictionary sensors do not use the optional description update hook.
# pylint: disable=protected-access,abstract-method,too-many-return-statements,too-many-branches
...
from .entity import MercedesMeEntity


def _create_sensor_if_eligible(
    key, config, car, coordinator, should_poll=False, initial_setup=False
):
...
async def create_missing_sensors_for_car(car, coordinator, async_add_entities):
    ...
    if missing_sensors:
        async_add_entities(missing_sensors, True)
...
async def async_setup_entry(
    ...
    for car in coordinator.client.cars.values():
        for key, value in sorted(SENSORS.items()):
            device = _create_sensor_if_eligible(
                key, value, car, coordinator, False, initial_setup=True
            )
            if device:
        ...
        for key, value in sorted(SENSORS_POLL.items()):
            device = _create_sensor_if_eligible(
                key, value, car, coordinator, True, initial_setup=True
            )
            if device:
                sensor_list.append(device)
    ...
    async def discover(car):
        await create_missing_sensors_for_car(car, coordinator, async_add_entities)

    coordinator.discovery_callbacks.append(discover)
