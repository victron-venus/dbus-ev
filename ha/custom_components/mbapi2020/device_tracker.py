# Dictionary entities do not use the optional description update hook.
...
async def async_setup_entry(
    ...
    async def discover(car):
        devices = []
        for key, value in DEVICE_TRACKER.items():
            device = MercedesMEDeviceTracker(
                internal_name=key,
                config=value,
                vin=car.finorvin,
            ...
            if (
                f"device_tracker.{device.unique_id}" not in car.sensors
                and device.device_retrieval_status() in ["VALID", "NOT_RECEIVED"]
            ):
...
class MercedesMEDeviceTracker(MercedesMeEntity, TrackerEntity, RestoreEntity):
    ...
    async def async_added_to_hass(self):
        self._car.add_sensor(f"device_tracker.{self.unique_id}")
        await super().async_added_to_hass()

    async def async_will_remove_from_hass(self):
        self._car.remove_sensor(f"device_tracker.{self.unique_id}")
